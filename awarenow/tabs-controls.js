...
function initializeInstance(config, navWrapElement) {
  ...
  // Check if this is a custom-tabs="menu" instance
  const isCustomTabsMenu = navWrap.hasAttribute('custom-tabs') && navWrap.getAttribute('custom-tabs') === 'menu';
  ...
  const getCurrentButton = () =>
    navWrap.querySelector(`${buttonSelector}.${currentClass}`);
  ...
  function positionBackground(target, instant = false) {
    ...
    // Apply stroke adjustment for custom-tabs="menu" instances
    if (isCustomTabsMenu) {
      left = left - 1;
      top = top - 1;
    }
...
// [Rest of your GSAP and tab rotator code remains unchanged...]
...
if (window.innerWidth > 991) {
  ...
  ScrollTrigger.create({
    ...
    animation: gsap
      ...
      .to(
        "[step-target='second'] [steps-child]",
        { opacity: 0, duration: 0.5 },
        0
      ) // Goes from 1 to 0, reverses to 1
      .to(
