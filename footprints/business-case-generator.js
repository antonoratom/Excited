...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!formManager.init()) return console.error("Form manager failed to initialize");

  // ===================================
  // UTILITY FUNCTIONS
  // ===================================
  
  const setElementState = (element, { required = false, tabindex = null, value = null, checked = null } = {}) => {
    required ? element.setAttribute('required', '') : element.removeAttribute('required');
    
    if (tabindex !== null) {
      tabindex === -1 ? element.setAttribute('tabindex', '-1') : element.removeAttribute('tabindex');
    }
  ...
  const setElementsState = (elements, state) => elements.forEach(el => setElementState(el, state));

  const queryAll = (parent, selector) => Array.from(parent.querySelectorAll(selector));
  
  const getTextInputs = (parent) => queryAll(parent, 'input, textarea, select')
    .filter(input => ['text', 'email', 'tel', 'number'].includes(input.type) || 
            ['TEXTAREA', 'SELECT'].includes(input.tagName));

  const triggerValidation = () => setTimeout(() => 
...
const clearConditionalWrap = (wrap) => {
  setElementsState(getTextInputs(wrap), { required: false, tabindex: -1, value: '' });
  
  queryAll(wrap, '.conditional-subvalue_group').forEach(group => {
    setElementsState(queryAll(group, '[type="radio"]'), { 
      required: false, tabindex: -1, checked: false 
    });
    setElementsState(getTextInputs(group), { required: false, tabindex: -1, value: '' });
  });
};

const handleNoneCheckbox = (groupName) => {
  const noneCheckbox = document.querySelector(`[name="${groupName}"][value="None"]`);
  if (!noneCheckbox) return;
  
  const handleToggle = (isNone, checkbox) => {
    if (!checkbox.checked) return;
    
    const others = queryAll(document, `[name="${groupName}"]`)
      .filter(cb => isNone ? cb.value !== 'None' : cb.value === 'None');
  ...
  queryAll(document, `[name="${groupName}"]`).forEach(cb => 
    cb.addEventListener('change', (e) => handleToggle(cb.value === 'None', e.target))
  );
};
  ...
  const handleConditionalCheckboxes = () => {
    queryAll(document, '.conditional-checkbox_wrap').forEach(wrap => {
      ...
      const directInputs = getTextInputs(wrap)
        .filter(input => !input.closest('.conditional-subvalue_group'));
      ...
      const updateState = () => {
        ...
        subvalueGroups.forEach(group => {
          ...
          if (!isChecked) {
            setElementsState(inputs, { required: false, tabindex: -1, value: '' });
          }
        });
  ...
  const handleSubvalueGroup = (group) => {
    ...
    const updateInputs = () => {
      const isYesChecked = group.querySelector('[type="radio"][value="Yes"]')?.checked || false;
  ...
  const getDatepickerValue = (input) => {
    ...
    if (value?.trim() && value !== placeholder && value !== dataPlaceholder) return value;
    return input.parentElement?.querySelector('input[type="hidden"]')?.value?.trim() || '';
  };
  ...
  const validateInput = (input, activePane) => {
    ...
    if (checkboxWrap?.querySelector('[type="checkbox"]')?.checked === false) return true;
    
    if (input.type !== 'radio') {
      const subvalueGroup = input.closest('.conditional-subvalue_group');
      if (subvalueGroup?.querySelector('[type="radio"][value="Yes"]')?.checked === false) return true;
    }
    ...
    if (input.type === 'radio') {
      ...
      const radioGroup = queryAll(activePane, `input[type="radio"][name="${radioName}"][required]`);
      return radioGroup.length === 0 || radioGroup.some(radio => radio.checked);
    }
  ...
  const validateCheckboxGroup = (checkboxes) => {
    const isValid = checkboxes.length === 0 || checkboxes.some(cb => cb.checked);
  ...
  const validateStep = (stepIndex, activePane, nextButton) => {
    ...
    const requiredInputs = queryAll(activePane, 'input[required], textarea[required], select[required]');
    const checkboxGroups = CHECKBOX_GROUPS.map(name => queryAll(activePane, `[name="${name}"]`));

    const checkValidity = () => {
      const [storeTypesValid, onSiteValid, offSiteValid] = checkboxGroups.map(validateCheckboxGroup);
      const allFilled = requiredInputs.every(input => validateInput(input, activePane));
      const isValid = allFilled && storeTypesValid && onSiteValid && offSiteValid;
      ...
      if (isValid) {
        ...
        const errorMessage = errorMessages.find(([valid]) => !valid)?.[1] || 
    ...
    [...checkboxGroups.flat(), ...queryAll(activePane, 'input[type="radio"]'), ...requiredInputs]
      .forEach(input => {
        const checkboxGroupName = input.getAttribute('name');
        
        if (input.type === 'radio' || CHECKBOX_GROUPS.includes(checkboxGroupName)) {
          input.addEventListener('change', checkValidity);
        } else {
          ["input", "blur"].forEach(event => input.addEventListener(event, checkValidity));
  ...
  const trackDropdownChanges = () => {
    queryAll(document, '.conditional-form_input-bl')
      ...
      .forEach(container => {
        ...
        if (!placeholder || !textInput || !dropdown || textInput.closest('#merged-selects')) return;
        ...
        const updateInput = () => {
          ...
          hasValue ? textInput.setAttribute("value", currentText) : textInput.removeAttribute("value");
